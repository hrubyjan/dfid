 ...
 * @author Marek Mikes
 */
public class IDBAwardedContractsGoodsAndWorksParser extends BaseDFIDTenderParser {
        ...
        for (int i = 0; i < rows.size(); i += 3) {
            ...
            parsedTender
                    ...
                    .setDonorFinancing(new ParsedPrice()
                            .setCurrency("USD")
                            .setNetAmount(parseTenderDonorFinancingNetAmount(contentRow)))
                    .addMajorSector(parseTenderMajorSector(contentRow));

            // parse common attributes
            parsedTender
                    .setSupplyType("GOODS & WORKS")
                    .addBuyer(new ParsedBody()
                            .setName(parseBuyerName(contentRow))
                    ...
                    .setProcedureType(parseTenderProcedureType(contentRow));
    ...
    /**
     * Parse buyer name value from row representing content of record.
     *
     * @param contentRow
     *         row representing content of record to be parsed
     *
     * @return String or Null
     */
    private static String parseBuyerName(final Element contentRow) {
        return JsoupUtils.selectText("table > tbody > tr:nth-child(2) > td:nth-child(2)", contentRow);
    }

    /**
     * Parse tender donor financing net amount value from row representing content of record.
     *
     * @param contentRow
     *         row representing content of record to be parsed
     *
     * @return String or Null
     */
    private static String parseTenderDonorFinancingNetAmount(final Element contentRow) {
        return JsoupUtils.selectText("table > tbody > tr:nth-child(2) > td:nth-child(4)", contentRow);
    }
    ...
    private static String parseTenderMajorSector(final Element contentRow) {
        return JsoupUtils.selectText("table > tbody > tr:nth-child(3) > td:nth-child(2)", contentRow);
    }
    ...
    /**
     * Parse tender procedure type value from row representing content of record.
     *
     * @param contentRow
     *         row representing content of record to be parsed
     *
     * @return String or Null
     */
    private static String parseTenderProcedureType(final Element contentRow) {
    ...
    private static ParsedAddress parseBidderAddress(final Element contentRow) {
        final String countryAndCityString =
                JsoupUtils.selectText("table > tbody > tr:nth-child(4) > td:nth-child(4)", contentRow);
